...
const getCalcScore = ({ startPx, movePx, percent }: { startPx: number, movePx: number, percent: number }) => {
...
const Star = ({ percent, index }: { percent: number, index: number }) => {
    ...
    return <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" overflow={'hidden'} style={{ position: 'relative' }}>
        <g clipPath="url(#clip0_712_541)" style={{ overflow: 'hidden', position: 'relative' }}>
            {/* outer 부분 (노란색) */}
            <motion.rect
                width={24} height={24}
                fill={'red'}
        ...
        <Flex pos={'relative'} className={classes['star-score-layout']}>
            <Flex bg={'white'} style={{ overflow: 'hidden' }}>
                {Array.from({ length: maxScore }).map((_, index) => {
                    const percent = (decimalScore >= index + 1) ? 1 : Math.max(0, score - index);
                    return <Star percent={percent > 1 ? 1 : percent} index={index} />;
                })}
            </Flex>
        </Flex>
    );
